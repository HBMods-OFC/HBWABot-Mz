...
exports.webp2mp4File = async (path) => {
    return new Promise((resolve, reject) => {
        if (!fs.existsSync(path)) return reject("File path a dik lo, thlalak a awm lo a ni thei.");
        ...
        axios({
            ...
            if (!file) {
                return reject("Ezgif-ah upload a hlawhtling lo. I file size a lian lutuk a ni thei.");
            }
            ...
            axios({
                ...
                const $ = cheerio.load(data);
                const source = $('div#output > p.outfile > video > source').attr('src');
                
                if (!source) {
                    return reject("Conversion a hlawhtling lo, a link kan hmu thei lo.");
                }

                const result = 'https:' + source;
                resolve({
                    status: true,
                    message: "Created by HBWABot",
                    result: result
                });
            }).catch(err => reject(`Step 2 Error: ${err.message}`));
        }).catch(err => reject(`Step 1 Error: ${err.message}`));
    });
};
